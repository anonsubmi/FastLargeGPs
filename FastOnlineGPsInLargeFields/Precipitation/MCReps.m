...
for N=Ns
    ...
    %Compute SMSEs and MMSEs
    SMSE=(mean((mu-ytest).^2))./std(ytest)^2;
    MSLL=1./2*mean((mu-ytest).^2./variance+log(2*pi*variance));
    
    %Write results to file
    fprintf(fileID,['N=',num2str(N),': ']);
    fprintf(fileID,['SMSE=',num2str(SMSE),', MSLL=',num2str(MSLL),', Runtime: ',num2str(time),'\n']);

    disp(['N=',num2str(N),': ']);
    disp(['SMSE=',num2str(SMSE),', MSLL=',num2str(MSLL),', Runtime: ',num2str(time)]);
end
...
for N=Ns
    ...
    fprintf(fileID,['SMSE=',num2str(SMSE),', MSLL=',num2str(MSLL)]);
    fprintf(fileID,['--Training time: ',num2str(mean(preprocessingtime)),', Inferencetime: ',num2str(mean(inferencetime)),'\n']);
    
    disp(['N=',num2str(N),', m=',num2str(m),': ']);
    disp(['SMSE=',num2str(SMSE),', MSLL=',num2str(MSLL)]);
    disp(['--Preprocessing time: ',num2str(mean(preprocessingtime)),', Inferencetime: ',num2str(mean(inferencetime))]);
...
for N=Ns
    ...
    N_u(1)=round(sqrt(m./N_u(3).*(L_1*l_SE(2))./(L_2*l_SE(1))));
    N_u(2)=floor(m./(N_u(1)*N_u(3))); 
    r=6.*l_SE; %Distance limiter for measurements used in LI inference
    dist=3.*l_SE;

    %Find the local approximation
    [mu, var, preprocessingtime, inferencetime]=FloatingDomainGP3D(X(1:N,:)',y(1:N)',Xtest',Omega,N_u,sigma_SE,l_SE,dist,r,sigma_y);
    ...
    fprintf(fileID,['N=',num2str(N),', m=',num2str(m),': ']);
    fprintf(fileID,['SMSE=',num2str(SMSE),', MSLL=',num2str(MSLL)]);
    fprintf(fileID,['--Preprocessing time: ',num2str(mean(preprocessingtime)),', Inferencetime: ',num2str(mean(inferencetime)),'\n']);
